...
export async function GET(req) {
    ...
    const tripDetails =`${tripInfo.data.city}, ${tripInfo.data.country} for period ${tripInfo.data.start_date} - ${tripInfo.data.end_date} at ${tripInfo.data.peace} peace (where relaxed is easy peace including calm activities, like for example city walkes, thrill-seeker looks for adrenaline activities and adventurer is keen to explore but has some common sense.) and a ${tripInfo.data.budget} level budget (where explorer is the economic one, comfort the normal one and indulge a luxury, adjust activities to the budget level, if someone chooses for example premium, make sure that they will receive unique and expensive activities suggestions.)`;
    const interestsList = interests.map(interest => interest.interest).join(", ");

    const messageContent = `
        You are a travel planner creating an intinerary for ${userInfo.data.name}'s trip. They are of age ${userInfo.data.age_group}, gender ${userInfo.data.gender} and they are of a traveler type ${userInfo.data.traveler_type} (solo - they are travelling alone or with family of with friends). The user has planned the following trip: ${tripDetails}. This user is interested in: ${interestsList} make sure that you take them into consideration while planning.. Please make in in markdown, as a list structured by day, with each day's activities under keys for Day 1, Day 2, etc. and times of the day: morning, afternoon evening. Adjust itinerary to all these preferences. Be specific - suggest actual places and be factual, for example 'National Art Museum of Catalonia - collection of catalan art'. Estimate expanses per day. Use the headline: Itinerary: 
    `;
